...
const submitForm = async () => {
  try {
    await store.createTier({ ...form.value })
    showModal.value = false
    form.value = { type: 'CLIENT', raisonSociale: '', email: '', telephone: '', typeClient: 'ENTREPRISE'}
  } catch(e) {
    console.error(e)
  }
}
...
<template>
  <MainLayout>
    ...
    <div class="tabs-nav">
      ...
      <button class="tab-btn" :class="{ active: activeTab === 'CLIENT' }" @click="activeTab = 'CLIENT'">Clients ({{ store.clients.length }})</button>
      <button class="tab-btn" :class="{ active: activeTab === 'FOURNISSEUR' }" @click="activeTab = 'FOURNISSEUR'">Fournisseurs ({{ store.fournisseurs.length }})</button>
    </div>

    <!-- Tableau -->
    <div class="table-card">
      <div v-if="store.loading && store.tiers.length === 0" class="loading-state">
        Chargement...
      </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-if="store.tiers.filter(t => activeTab === 'TOUS' || t.type === activeTab).length === 0" class="empty-row text-center">
            <td colspan="5">Aucun tier trouvé.</td>
          </tr>
          
          <tr v-for="item in store.tiers.filter(t => activeTab === 'TOUS' || t.type === activeTab)" :key="item.id">
            <td class="font-semibold text-dark">{{ item.codeTiers }}</td>
            <td>
              <span class="badge" :class="item.type === 'CLIENT' ? 'badge-client' : 'badge-fournisseur'">
                {{ item.type }}
              </span>
            </td>
            <td>
              <div class="motif-cell">
                ...
                <span class="text-muted" style="font-size: 0.75rem;">{{ item.email || 'Pas d\'email' }}</span>
              </div>
            </td>
            <td>{{ item.telephone || '---' }}</td>
            <td class="text-right font-semibold text-dark">{{ item.solde?.toLocaleString() || '0' }} XAF</td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="modal">
        ...
        <form @submit.prevent="submitForm" class="modal-body">
          ...
          <div class="form-group">
             ...
             <input v-model="form.raisonSociale" type="text" required class="input-std" placeholder="Nom de l'entreprise ou personne" />
          </div>
          
          <div class="form-row">
            <div class="form-group half">
               <label>Téléphone</label>
               <input v-model="form.telephone" type="text" class="input-std" placeholder="+237 6..." />
            </div>
            <div class="form-group half">
               <label>Email</label>
               <input v-model="form.email" type="email" class="input-std" placeholder="contact@..." />
            </div>
          </div>
          ...
          <div class="modal-footer pt-4 pb-0">
             ...
             <button type="submit" class="btn-primary" :disabled="!form.raisonSociale || store.loading">
...
.motif-cell { display: flex; flex-direction: column; gap: 0.15rem; }
...
.badge-fournisseur { background: #fef3c7; color: #d97706; }

/* TABS NAV */
.tabs-nav { display: flex; gap: 1rem; border-bottom: 1px solid #e5e7eb; margin-bottom: 1.5rem; border-radius:0;}
.tab-btn { background: none; border: none; padding: 0.75rem 0.5rem; font-size: 0.875rem; font-weight: 600; color: #6b7280; border-bottom: 2px solid transparent; cursor: pointer; transition: color 0.15s;}
...
.modal { background: white; border-radius: 12px; width: 100%; max-width: 500px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); }
